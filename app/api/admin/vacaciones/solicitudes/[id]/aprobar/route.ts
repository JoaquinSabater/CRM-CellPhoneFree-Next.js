...
export async function PATCH(
  ...
  try {
    ...
    if (!saldo || saldo.dias_pendientes < solicitud.cantidad_dias) {
      await connection.rollback();
      return jsonError('El saldo del usuario no es consistente para aprobar esta solicitud.', 409);
    }
    ...
    await connection.rollback();
    console.error('[admin/vacaciones/aprobar] Error:', error);
    return jsonError('No se pudo aprobar la solicitud.', 500);
  } finally {
    connection.release();
  }
}
